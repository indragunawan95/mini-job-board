    ...
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">
          {mode === 'dashboard' ? "User's Jobs Posting" : 'Browse Jobs'}
        </h1>
          ...
          <tbody>
            {isLoading ? (
              ...
              jobs.map((job) => (
                <tr key={job.id}>
                  <td
                    className="hover:bg-primary-50 hover:cursor-pointer hover:opacity-90"
                  ...
                  <td>
                    <span className="badge badge-ghost badge-sm">{job.job_type}</span>
                  </td>
                  {mode === 'dashboard' && user && user.id === job.user_id && (
                    <td>
                      <div className="flex gap-2">
                        <Link href={`/jobs/${job.id}/edit`} className="btn btn-ghost btn-xs">
                          Edit
                        </Link>
                        <button
